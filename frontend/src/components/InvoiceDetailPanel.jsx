...
export default function InvoiceDetailPanel({ invoice, onClose }) {
  ...
  return (
    <>
      <button
        type="button"
        className="fixed inset-0 bg-[var(--bg-overlay)] z-20 sm:hidden"
        onClick={onClose}
        aria-label="Close invoice details"
      />
      <div className="fixed right-0 top-0 z-30 h-full w-full max-w-full sm:w-80 bg-[var(--bg-surface)] border-l border-[color:var(--border)] shadow-xl overflow-y-auto">
                  ...
                  <>
                    <div className="flex gap-2">
                      <input
                        readOnly
                        value={paymentLink}
                        className="min-w-0 flex-1 border border-[color:var(--border-muted)] rounded-lg px-3 py-2 text-xs bg-[var(--bg-surface-muted)] text-[var(--text-primary)] truncate"
                      />
                      <button
                        onClick={handleCopy}
